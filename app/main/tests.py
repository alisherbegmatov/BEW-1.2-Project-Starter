...
class MainTests(unittest.TestCase):
    ...
    def test_create_task(self):
        ...
        post_data = {
            "title": "Final Project",
            "description": "Should Have Models, Forms, unit tests",
            "status": "In Progress",
        ...
        created_task = Task.query.filter_by(title="Final Project").one()
        self.assertIsNotNone(created_task)
        self.assertEqual(created_task.description,
                         "Should Have Models, Forms, unit tests")
    ...
    def test_view_task(self):
        ...
        response_text = response.get_data(as_text=True)
        self.assertIn("apply to Apple", response_text)
        self.assertIn("iOS Team", response_text)
        self.assertIn("In Progress", response_text)
        self.assertIn("2022-03-03", response_text)
    ...
    def test_update_board(self):
        ...
        post_data = {"title": "Job Searching", "user": user}
        self.app.post(f"/update-taskBoard/{board.id}", data=post_data)

        update_board = TaskBoard.query.filter_by(title="Job Searching").one()
        self.assertIsNotNone(update_board)
        self.assertEqual(update_board.title, "Job Searching")
    ...
    def test_update_task(self):
        ...
        post_data = {
            ...
            "description": "Should Have Models, Forms, unit tests",
            "status": "Archived",
            "due_date": date(2022, 3, 3),
            "board": board,
        }
        self.app.post(f"/update_task/{board.id}/{task.id}", data=post_data)

        update_task = Task.query.filter_by(title="Final Project").one()
